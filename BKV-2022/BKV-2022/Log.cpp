...
namespace Log
{
	...
	void WriteError(LOG log, Error::ERROR error)
	{
		...
		{
			*log.stream << "ОШИБКА " << error.id << ": " << error.message << "\n" << "Строка " << error.errorPosition.line << ", позиция: " << error.errorPosition.col << endl;
		}
		log.stream->close();
	}
