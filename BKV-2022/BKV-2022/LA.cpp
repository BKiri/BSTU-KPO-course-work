	...
	for (int i = 0, j = 0; i < source.size; i++)
	{
		...
		else
		{
			if (j != 0)
			{
				i--;
				temp[j] = '\0';				
				if (TokenAnalyse(temp, str_number, lexTable, idTable))
				{
					temp[0] = '\0'; j = 0;
					continue;
				}
...
bool TokenAnalyse(char* token, const int str_number, LT::LexTable& lexTable, IT::IdTable& idTable)
{
	...
	case 'f':
	{
		...
		else
		{
			...
			return Funcs(token, str_number, lexTable, idTable, flag_type_variable);
		}
	}
	case 'i':
	{
		FST::FST* integer = new FST::FST(INTEGER(token));
		FST::FST* _for = new FST::FST(IF(token));
		...
		else if (FST::execute(*_for))
		{
			...
			Add(lexTable, { LEX_IF,str_number,idTable.size - 1 });
...
bool Funcs(char* token, const int str_number, LT::LexTable& lexTable, IT::IdTable& idTable, Flag_type_variable& flag_type_variable)
{
	...
	if (FST::execute(*id))
	{
				...
				flag_type_variable.LT_posititon == lexTable.size - 2))
		{
			...
			else if ((strcmp(token, "strl") == 0  && idTable.IsId(token) != -1))
			{
					if (strcmp(token, "strl") == 0)
					{
						idTable.Add({ "\0", token, IT::IDDATATYPE::INT, IT::IDTYPE::F });
		...
		if (!Checked_id && (lexTable.GetEntry(lexTable.size - 1).lexema == 't' && flag_type_variable.LT_posititon == lexTable.size - 1))
		{
			...
			for (int i = lexTable.size - 1; i >= 0; i--)
			{
				...
				if (LEXEMA == LEX_IF && LeftBrace && RightBrace)
				{
					LeftBrace = false;
					RightBrace = false;
					continue;
				...
				if ((LeftBrace && !RightBrace &&
					...
					((LEXEMA == LEX_ID && strcmp(idTable.GetEntry(lexTable.GetEntry(i).idxTI).id,"main") == 0)))|| (LEXEMA == LEX_IF && (!LeftBrace && !RightBrace))
					)
				{
					if (idTable.IsId(token, idTable.GetEntry(lexTable.GetEntry(i).idxTI).id,i) == -1)
					{
		...
		if (!Checked_id)
		{
			...
			for (int i = lexTable.size - 1; i >= 0; i--)
			{
				...
				if (LEXEMA == LEX_IF && LeftBrace && RightBrace && LeftThesis && RightThesis)
				{
					LeftBrace = false;
					RightBrace = false;
					LeftThesis = false;
				...
				if ((LeftBrace && !RightBrace && ((LEXEMA == LEX_ID  && idTable.GetEntry(lexTable.GetEntry(i).idxTI).idtype == IT::IDTYPE::F))) || 
					(LEXEMA == LEX_IF && (LeftBrace && !RightBrace) || (LEXEMA == LEX_IF && (LeftThesis && !RightThesis))))
				{
					int temp = 0;
					IT::Entry id_f;
							...
							for (; iter >= 0; iter--)
							{
								if (lexTable.GetEntry(iter).idxTI != -1 && iter >= 0)
									if (idTable.table[lexTable.GetEntry(iter).idxTI].idtype == IT::F)
										break;
								if (iter >= 0)
								if (lexTable.GetEntry(iter).lexema == LEX_IF)
									break;
							}
							if (iter >= 0)
